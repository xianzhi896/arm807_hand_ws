...
class DetectionInfo {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize the length for message field [file_paths]
    bufferOffset = _serializer.uint32(obj.file_paths.length, buffer, bufferOffset);
    obj.file_paths.forEach((val) => {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize array length for message field [file_paths]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.file_paths = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.file_paths[i] = std_msgs.msg.String.deserialize(buffer, bufferOffset)
    }
  ...
  static Resolve(msg) {
    ...
    if (msg.file_paths !== undefined) {
      resolved.file_paths = new Array(msg.file_paths.length);
      for (let i = 0; i < resolved.file_paths.length; ++i) {
        resolved.file_paths[i] = std_msgs.msg.String.Resolve(msg.file_paths[i]);
      }
    }
    else {
      resolved.file_paths = []
